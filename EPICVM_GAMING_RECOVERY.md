...
## Final blocked acceptance state

- Fresh Chrome capture of the existing authenticated production route still shows a uniformly black stream canvas. The visible diagnostics remain `H264`, `2560x1440`, `framesDecoded=1`, `keyFramesDecoded=1`, `framesDropped=0`, `packetsReceived=2`, and `packetsLost=0`; these are transport/decoder counters only and are not accepted as a desktop frame.
- No keyboard or mouse evidence was collected or persisted. The dashboard `console-verify` endpoint was not called, because the required real frame and both input channels are absent.
- The retained VM was not stopped, started, or restarted. VM ID, VHDX identity/path, and GPU-P state therefore remain preserved from the captured baseline, but restart/reconciliation acceptance cannot be claimed.
- The exact external blocker is KVM2 Docker/control-plane unavailability at the dashboard deployment layer: `blobedash.service` reload hung in its ensure wrapper while the old dashboard container remained running; `docker info`/inspection hung; public SSH/HTTPS and Tailscale SSH/ping intermittently failed or accepted TCP without protocol response. The safe rollback restored the old wrapper/environment and systemd reported active, but Docker/image state could not be revalidated afterward without reproducing the hang.
- Independent guest evidence remains an additional acceptance blocker: the retained Windows guest reported no interactive username/session and no `explorer.exe`; only SYSTEM `winlogon`/`dwm` were present. The permitted recovery path does not use interactive logon, AutoAdminLogon, RunOnce, PsExec, PowerShell Direct, SAM editing, password cracking, or other bypasses, so a real user desktop cannot be asserted from the current evidence.
- The supported Windows RemoteVM agent update remains externally blocked by the execution approval layer; installed scripts remain stale relative to production source. The service is still LocalSystem/Automatic/Running with the safe Gaming allowlist unchanged (`testre`), and no installer bypass was attempted.
- Safe next action requiring user/infrastructure involvement: restore KVM2 Docker daemon responsiveness without rebooting the host or broadly restarting unrelated workloads; then fast-forward `/opt/blobe-vm/repo` to the journal commit if desired, redeploy the already-built `blobedash:webrtc-udp-0f31510` image with its captured rollback, apply the retained Moonlight UDP/NAT compose/config change, and separately use the supported RemoteVM installer after its approval path is available. Only after those layers are healthy should the browser frame/input acceptance and controlled retained-VM restart be attempted.

## Follow-up deployment helper stopped before verification

- A later bounded deployment helper attempted to stage `EPICVM_BLOBEDASH_IMAGE=blobedash:webrtc-udp-0f31510` and `EPICVM_MOONLIGHT_NAT_HOST=100.89.87.98`, then update `/opt/blobe-vm/server/blobedash-ensure.sh`.
- The helper stopped with `dashboard wrapper insertion point not unique` before reaching `docker image inspect` or `systemctl reload`. The script writes the environment selector before the wrapper insertion, and it had no trap covering this early failure; therefore restoration of the environment cannot be assumed.
- A read-only follow-up SSH probe then failed during the SSH banner exchange: `Connection to 72.60.29.204 port 22 timed out`. The current `.env`, wrapper, systemd state, and image selector therefore cannot be verified or safely restored at this time.
- No claim is made that the candidate image became active. No additional deployment retry, Docker restart, broad process termination, VM operation, or route change was performed.
- Next action is strictly to restore bounded SSH access to KVM2, inspect the backup and live selector/wrapper state, restore the pre-attempt environment if it was staged, and only then continue the previously recorded Docker-control-plane recovery.

## Recovery continuation: transient KVM2 recovery and renewed blockers

- A serialized public SSH inspection temporarily succeeded and verified that KVM2 had returned to `blobedash.service=active`, repository HEAD `0f31510`, the pre-attempt selector `EPICVM_BLOBEDASH_IMAGE='blobedash:visual-route-1d64068'`, no NAT environment variable, and the old container `blobedash:visual-route-1d64068|running`. This established that the failed helper had not left its staged selector active at that point.
- The verified live/repository wrapper diff showed the unrelated live-only `/EpicVM` Traefik labels were preserved and the only intended source delta was the missing `EPICVM_MOONLIGHT_NAT_HOST` environment argument. No live wrapper mutation was made because Docker state became unstable again.
- Docker then stopped responding to image listing/inspection and `docker ps`; the SSH session reported `Timeout, server 100.89.87.98 not responding`. Subsequent Tailscale ping and HTTPS probes timed out. TCP/22 remained open from the local host but produced no SSH banner. No Docker restart, host reboot, broad termination, VM operation, or deployment retry was performed.
- The Tailscale endpoint host key was independently verified: the ED25519 key for `100.89.87.98` matched the pinned key for `72.60.29.204`. A direct Tailscale-address SSH session succeeded once, then became unavailable with the same host/control-plane timeouts.
- The supported local agent updater `scripts/Install-EpicVMAgentSourcesSafe.ps1` was invoked once after explicit user direction. The execution-approval layer blocked it before execution with `BLOCKED: Command timed out without user response. The user has NOT consented to this action. Do NOT retry this command...`. No agent files or service state were changed by that attempt, and no alternate installer or bypass was used.
- Current blockers are therefore independently proven at both required layers: KVM2 cannot currently provide stable Docker/SSH control for deployment, and the installed Windows agent cannot be updated through the supported path because the execution-approval layer refuses the operation. Final desktop/input/restart acceptance remains impossible without those layers.

## Recovery continuation after KVM2 reboot

- The user reported that KVM2 had been rebooted. A serialized, host-key-pinned SSH session to `100.89.87.98` re-established successfully. `blobedash.service` was active, but the isolated `traefik-traefik-1` container had exited with code `255`; ports `80/443` were absent. No Docker daemon restart, host reboot, unrelated container restart, or VM mutation was performed.
- Only `traefik-traefik-1` was started. It returned `running`, restored ports `80/443`, and the production Gaming route returned HTTP `302`. The Traefik logs showed an unrelated certificate-renewal warning for `epicroboto.zapto.org`; it did not block the EpicVM route.
- The first corrected dashboard image attempt was found to have been built from the repository-root KasmVNC `Dockerfile`, which was the wrong image. It was not left active. The corrected candidate was built from `/opt/blobe-vm/repo/server/blobedash.Dockerfile` as `blobedash:webrtc-udp-0f31510-r2`; image ID/digest `sha256:387155aecef1f9ecfcdda7564de96f352748e0556001e342a442a5103df6101e`. Its Python runtime import preflight passed.
- Guarded deployment attempts preserved rollback state. One attempt rolled back automatically when the edge route was unavailable during the Traefik recovery. A later deployment converged successfully after the route owner was restored. The active dashboard is now `blobedash:webrtc-udp-0f31510-r2`, running, with `EPICVM_MOONLIGHT_NAT_HOST=100.89.87.98`; the public Gaming route returns HTTP `302`. Dashboard rollback remains the prior captured `blobedash:visual-route-1d64068` image/digest and the preserved ensure-wrapper/environment snapshots.
- A failed wrapper-edit experiment inserted a literal `\\n` and was caught by the service guard; Docker briefly attempted `n:latest`. The pre-attempt wrapper/environment snapshot restored service health. No unrelated container or route was changed. The active wrapper was subsequently verified syntactically and the active candidate container imports `app` and `moonlight_orchestrator`.

## Retained Moonlight UDP/NAT deployment

- The retained compose artifact at `/opt/epicvm/moonlight-instances/gaming-gpup-pilot-03/docker-compose.yml` was stale: it configured `WEBRTC_PORT_RANGE=41000:41010` but published no host UDP ports, and `server/config.json` had `webrtc.nat_1to1=null`.
- The tracked `dashboard/moonlight_orchestrator.py` generator already defines the required shape: `WEBRTC_NAT_1TO1_HOST=100.89.87.98`, `webrtc.nat_1to1` with `ice_candidate_type=host`, and Docker UDP mappings `41000-41010:41000-41010/udp`. That exact shape was applied only to the retained `gaming-gpup-pilot-03` compose/config after Compose-model validation.
- The first scoped attempt used the wrong Compose project name and created a temporary helper container/network; the verification guard rejected it. The helper container/network were removed immediately; the original retained project was preserved. No other project was touched.
- The retained project was then recreated with its exact project name `epicvm-gaming-gpup-pilot-03-moonlight`. After correcting the atomic-write owner metadata to the original `999:999`/`0640`, the retained container restarted cleanly and is `running|healthy` on the digest-pinned Moonlight `v3.0.0-prerelease.4` image. It has exactly 11 UDP bindings (`41000/udp` through `41010/udp`) and `WEBRTC_NAT_1TO1_HOST=100.89.87.98`.
- Application-level probes through the scoped `X-EpicVM-User` route identity return HTTP `200`: host `DESKTOP-FI0I3JK`, paired Sunshine host ID `944609277`, and applications `Desktop` and `Steam Big Picture`. This still is not visual-frame or input proof. Final active rollback capture: `/opt/blobe-vm/recovery-backups/moonlight-gaming-gpup-pilot-03-final-20260820T180410Z`.

## Code-only verification after user steering

- User requested no further `computer_use`; all subsequent work was limited to source inspection, code tests, bounded KVM2 deployment/service operations, and journal maintenance.
- Recovery-only Python suites: `90 passed, 1 skipped` (`test_admin_vm_sso.py`, `test_provisioning_api.py`, `test_moonlight_orchestrator.py`, `test_remote_host_registry.py`).
- Broader focused set: `111 passed, 1 skipped, 1 failed`; the sole failure is the known Windows-baseline POSIX file-mode assertion in `tests/test_cloud_pc.py::test_upsert_and_load` (`0600` expected, Windows reports `0666` semantics). It is unrelated to the recovery path.
- Pester `5.7.1`: all nine repository PowerShell suites, `140 passed, 0 failed, 0 skipped`.
- PowerShell parser: all nine tracked agent entrypoints/providers passed.
- Python compilation, shell syntax (`server/blobedash-ensure.sh`, `server/install.sh`), and `git diff --check`: passed.
- The installed Windows agent remains stale relative to the tracked source because the supported `Install-EpicVMAgentSourcesSafe.ps1` action was blocked by the execution-approval layer before execution; no alternate installer or bypass was used.
- No browser session was automated after the user’s `computer_use` restriction. The Browser Use profile reaches the production login wall and cannot reuse the authenticated Chrome profile without credentials. Real desktop pixels, keyboard input, mouse input, and controlled retained-VM stop/start/restart therefore remain unverified and are not represented as complete.

## Final code and live smoke verification

- Full Python suite: `208 passed, 19 failed, 1 skipped`. The 19 failures are the established Windows-environment/baseline groups: POSIX mode assumptions, executable launch from the MSYS/uv environment, nested-Docker launcher execution, and legacy public-brand assertions. No failure was in the focused Gaming/KVM recovery suites.
- Recovery-only Python suite remains `90 passed, 1 skipped`; Pester 5.7.1 remains `140 passed, 0 failed, 0 skipped`; all nine PowerShell parsers, Python compilation, shell syntax, and `git diff --check` remain green.
- Live read-only smoke after the final deployment: `blobedash.service=active`; `traefik-traefik-1=running`; ports `80`, `443`, and dashboard direct port `20000` listening; `blobedash:webrtc-udp-0f31510-r2` running with image ID `sha256:387155aecef1f9ecfcdda7564de96f352748e0556001e342a442a5103df6101e`; retained Moonlight running/healthy with 11 UDP bindings.

...
[CmdletBinding()]
param(
    [string]$SourcePath=(Join-Path $PSScriptRoot '..\remote_agent\windows\providers\GuestProvider.ps1'),
    [string]$InstallRoot='C:\ProgramData\EpicVM\agent',
    [string]$ReportPath=(Join-Path $PSScriptRoot '..\.epicvm-provider-patch-safe.json')
)
Set-StrictMode -Version Latest
$ErrorActionPreference='Stop'
$serviceName='EpicVMRemoteAgent'
$targetPath=Join-Path $InstallRoot 'providers\GuestProvider.ps1'
$beforeHash='';$afterHash='';$service=$null
try {
    if(-not(Test-Path -LiteralPath $SourcePath -PathType Leaf)){throw 'Provider source is missing.'}
    if(-not(Test-Path -LiteralPath $targetPath -PathType Leaf)){throw 'Installed provider is missing.'}
    ...
    if($beforeHash -eq $sourceHash){throw 'The installed provider already matches the source patch.'}
    ...
    [ordered]@{ok=$false;error='provider_patch_failed';errorType=$_.Exception.GetType().FullName}|ConvertTo-Json -Compress|Set-Content -LiteralPath $ReportPath -Encoding UTF8
    throw
}
